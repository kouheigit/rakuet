...
　　　　　　　　@if ($hantei ?? '' != '')
		<p class="kisosetu">基礎代謝とは何もしないで過ごしていても消費するカロリー数の事です、極端に言うと一日寝ていただけでも消費されるカロリーの事を言います。{{$name ?? ''}}さんの基礎代謝は{{$BMR1 ?? ''}}kcalになります。{{$name ?? ''}}さんの生活スタイルの運動量が多い生活例えば通勤まで１時間以上とか自転車通勤であれば{{$BMR1 ?? ''}}kcalに＋300kcalの{{$BMR2 ?? ''}}kcalの運動量が少ない生活であれば{{$BMR1 ?? ''}}kcalに＋100kcalの{{$BMR3 ?? ''}}kcalが一日の{{$name ?? ''}}さんの消費カロリーとなります</p>
		 @else
	         <p></p>
                @endif
                <div id="homeform">
